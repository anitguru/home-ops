...
def concise_recall(summary: dict[str, Any], grok_feedback: str, apply_results: list[str], metrics_push_result: str) -> str:
    ...
    lines = [
        ...
        "- SOPS+age used: yes (social/X + Postgres secrets loaded via sops_env.py)",
        "- X metrics source: Tweepy/X API credentials from the local SOPS store, not the xurl CLI",
