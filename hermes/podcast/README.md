...
- If Chatterbox hangs on a long request, keep using `scripts/chatterbox_tts_segments.sh`; do not revert to single-request TTS without revalidating. Chatterbox can be healthy/listening but slow enough that a segment exceeds a short client timeout; keep per-segment curl `--max-time` comfortably above observed outliers (2026-06-11 hit ~14 minutes for one 362-character segment while the service stayed up).
