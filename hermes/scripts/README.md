...
- `wiki_freshness_cron.sh` — default-profile Hermes no-agent cron launcher for backup-first wiki maintenance and freshness checks under `hermes/wiki-freshness/`. The active scheduled path deterministically repairs unambiguous schema/frontmatter/path/index issues, then runs a live no-LLM URL audit; ambiguous moves fail safe. It avoids Gitea Actions, runners, Git push/writeback, and direct Claude/Anthropic APIs.
